...
| 零感知使用路径（意图输入优先，AI 隐式路由流程） | Approved Now | 这是产品北极星，但应作为 Phase 2/3 的设计主线推进，不能抢在可靠性硬化之前 |
...
**为什么现在做**  
这不是一个“文档优化”点子，而是产品定位收敛：NexusRhythm 的规则本来就主要是给 AI 看的，而不是要求普通用户先学会 Claude Code 的全部能力模型再开始。

**计划落点**
- 先写正式设计文档，明确“哪些概念必须暴露给人，哪些应该藏在 AI 默认行为里”
- 作为 Phase 2 / Phase 3 的产品主线，而不是插队打断当前可靠性加固
- 保留 power user 的显式命令路径，不为了新手体验牺牲熟练用户效率
...
2. 再做一个 `doctor` 自检入口
3. 再明确“零感知使用”策略，把新手体验作为后续产品主线
4. 然后试点迁移一个 command 到 skill
5. 最后再做文档分层和分发增强
