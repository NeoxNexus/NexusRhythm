# WALKTHROUGH — Phase 3: v0.2 空白项目 bootstrap 验证
...
这个 Phase 3 现在被重新归档为 `v0.2` 的最后一道验收关，而不是单独的“产品化主线”。它交付了一个能在干净目录里真正生成的 demo workspace：脚本会创建最小 Python 示例项目、写入 demo 自己的 `ROADMAP.md`、调用 `install.sh` 注入 NexusRhythm，然后让生成后的工程默认通过 `doctor quick` 和 `gate-check`。它验证的是“空白项目安装后能不能跑通”，而不是更大范围的对外发布材料。
...
- [x] 新增 Phase 3 SPEC：`docs/specs/SPEC_PHASE_3_demo-workspace-bootstrap.md`

---

## 2. 关键技术决策

### 决策一：空白项目验收采用“生成式 demo workspace”，而不是提交一整份静态 demo 仓库
- **背景**：静态 demo 很容易和脚手架自身演进脱节，后续维护成本高。
- **选择**：新增脚本在目标目录生成最小宿主项目，并复用现有 `install.sh` 注入脚手架。
- **原因**：这样 demo 走的就是未来真实用户的安装路径，验收价值更高。

### 决策二：demo workspace 先写自己的 ROADMAP，再执行安装
- **背景**：当前 `install.sh` 仍会复制仓库自身的 `ROADMAP.md`，这对验收样本会造成状态污染。
- **选择**：在 demo 生成器里先写入 demo 自己的最小 `ROADMAP.md`，再运行安装脚本。
- **原因**：这样能先保证空白项目验证真实可信，同时把更广泛的安装模板问题留作后续治理项。

### 决策三：宿主示例项目保持最小 Python + unittest 组合
- **背景**：这个阶段的职责被收敛为 `v0.2` 的安装后验收，而不是展示多语言模板能力。
...
| 3 | 如果示例工程依赖 pytest 或第三方包，干净环境验证会变脆弱 | 安装后验收阶段过早引入环境依赖 | 先用标准库 `unittest` 保持零额外依赖 |
...
- 先把“空白项目 bootstrap 可生成且可验证”当成 `v0.2` 收口基线，再决定是否扩更高层产品化体验
- `install.sh` 复制根级 ROADMAP 的问题已经暴露，后续应收敛为更中立的项目模板方案
- 下一步主线应该切回 `v0.3` 的任务层，而不是继续把更高层产品化能力塞进历史的 Phase 3

---

## 7. 小复盘

**最大意外**：真正让 `v0.2` 收口更扎实的，不是写更多宣传文案，而是让空白项目示例可以被脚本稳定生成。

**最大收获 / 教训**：安装后验收最有价值的时候，不是“长得像产品”，而是“真的复用了未来用户会走的那条安装路径”。
